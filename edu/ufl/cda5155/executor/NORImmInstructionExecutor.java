...
public class NORImmInstructionExecutor implements Executor<InstructionExecutionResult> {
  ...
  public InstructionExecutionResult execute(Instruction instruction, ProgramCounter programCounter, GlobalData registers) {
    ...
    //		registers.setRegisterValue(rt.getId(), ~(registers.getRegisterValue(rs.getId()) | instruction.getImmediate()));
    GlobalData gb = EntityManager.getInstance().getGlobalData();
    return new InstructionExecutionResult(instruction, rt.getId(), ~(gb.getRegister(rs.getId()).getValue()) | instruction.getImmediate());
  }
}
