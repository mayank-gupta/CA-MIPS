...
public class SLTInstructionExecutor implements Executor<InstructionExecutionResult> {
  ...
  public InstructionExecutionResult execute(Instruction instruction, ProgramCounter programCounter,	GlobalData registers) {
    ...
    //		registers.setRegisterValue(rd.getId(), registers.getRegisterValue(rs.getId()) < registers.getRegisterValue(rt.getId()) ? 1 : 0);
    GlobalData gb = EntityManager.getInstance().getGlobalData();
    return new InstructionExecutionResult(instruction, rd.getId(), gb.getRegister(rs.getId()).getValue() < gb.getRegister(rt.getId()).getValue() ? 1 : 0);
  }
}
