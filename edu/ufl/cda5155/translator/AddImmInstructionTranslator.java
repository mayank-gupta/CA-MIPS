...
public class AddImmInstructionTranslator implements Translator {

  @Override
  public boolean doesSatisfy(String instructionBinary) {
    return instructionBinary.substring(32-6, 32).matches("000011");
  }
  ...
  public Instruction translate(String instructionBinary) {
    ...
    instruction.setStringRepresentation("ADD R" + instruction.getRt().getId() + ", R" + instruction.getRs().getId() + ", #" + instruction.getImmediate());
    instruction.setOperator(Operator.ADDI);
    return instruction;
  }
