...
public class SubInstructionTranslator implements Translator {

  @Override
  public boolean doesSatisfy(String instructionBinary) {
    return instructionBinary.substring(0, 6).matches("010001");
  }
  ...
  public Instruction translate(String instructionBinary) {
    ...
    instruction.setStringRepresentation("SUB R" + instruction.getRd().getId() +", R" + instruction.getRs().getId() +", R" + instruction.getRt().getId());
    instruction.setOperator(Operator.SUB);
    return instruction;
  }
