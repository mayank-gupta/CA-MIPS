...
public class MulInstructionTranslator implements Translator {

  @Override
  public boolean doesSatisfy(String instructionBinary) {
    return instructionBinary.substring(0, 6).matches("010000") && 
      instructionBinary.substring(32-6, 32).matches("001110");
  }
  ...
  public Instruction translate(String instructionBinary) {
    ...
    instruction.setStringRepresentation("MUL R" + instruction.getRd().getId() +", R" + instruction.getRs().getId() +", R" + instruction.getRt().getId());
    instruction.setOperator(Operator.MUL);
    return instruction;
  }
