...
public class SRAInstructionTranslator implements Translator {

  @Override
  public boolean doesSatisfy(String instructionBinary) {
    return instructionBinary.substring(0, 6).matches("110000");
  }
  ...
  public Instruction translate(String instructionBinary) {
    ...
    instruction.setStringRepresentation("SRA R" + instruction.getRd().getId() + ", R" + instruction.getRt().getId() + ", #" + InstructionUtil.binaryToDecimal(instruction.getSa()));
    instruction.setOperator(Operator.SHIFT_RIGHT_ARITHMETIC);
    return instruction;
  }
