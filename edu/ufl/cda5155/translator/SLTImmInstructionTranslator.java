...
public class SLTImmInstructionTranslator implements Translator {

  @Override
  public boolean doesSatisfy(String instructionBinary) {
    return instructionBinary.substring(32-6, 32).matches("101011");
  }
  ...
  public Instruction translate(String instructionBinary) {
    ...
    instruction.setStringRepresentation("SLT R" + instruction.getRt().getId() + ", R" + instruction.getRs().getId() + ", #" + instruction.getImmediate());
    instruction.setOperator(Operator.SLTI);
    return instruction;
  }
