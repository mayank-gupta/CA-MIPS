...
public class SRLInstructionTranslator implements Translator {

  @Override
  public boolean doesSatisfy(String instructionBinary) {
    return instructionBinary.substring(0, 6).matches("010000") && 
      instructionBinary.substring(32-6, 32).matches("000000");
  }
  ...
  public Instruction translate(String instructionBinary) {
    ...
    instruction.setStringRepresentation("SRL R" + instruction.getRd().getId() + ", R" + instruction.getRt().getId() + ", #" + InstructionUtil.binaryToDecimal(instruction.getSa()));
    instruction.setOperator(Operator.SHIFT_RIGHT_LOGICAL);
    return instruction;
  }
