...
public class NORInstructionTranslator implements Translator {

  @Override
  public boolean doesSatisfy(String instructionBinary) {
    return instructionBinary.substring(0, 6).matches("111001");
  }
  ...
  public Instruction translate(String instructionBinary) {
    ...
    instruction.setStringRepresentation("NOR R" + instruction.getRd().getId() +", R" + instruction.getRs().getId() +", R" + instruction.getRt().getId());
    instruction.setOperator(Operator.NOR);
    return instruction;
  }
