...
public class BLTZInstructionTranslator implements Translator {

  @Override
  public boolean doesSatisfy(String instructionBinary) {
    return instructionBinary.substring(32-6, 32).matches("100000") && 
      instructionBinary.substring(16, 21).matches("00000");
  }
  ...
  public Instruction translate(String instructionBinary) {
    ...
    instruction.setStringRepresentation("BLTZ R" + instruction.getRs().getId() +", #" + (InstructionUtil.binaryToDecimal(instruction.getOffset()) << 2));
    instruction.setOperator(Operator.BRANCH_ON_L_T_ZERO);
    return instruction;
  }
